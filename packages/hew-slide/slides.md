...
- Hello CI/CD
  - 初めての本気で作り込むCI/CD
  - Infrastructure as Codeで作る、お手軽構築
-->
...
![users](https://misskey.na2na.dev/media/media/068a282d-afd0-446a-8540-9724d2d5af33.png)
...
![messages](https://misskey.na2na.dev/media/media/a752c1af-13fe-40cb-86d1-3ac44702c785.png)
...
![messages](https://misskey.na2na.dev/media/media/be53dfcc-aa8c-4656-bfb6-a52899cd14b5.png)
...
[Swagger UI](https://oas-sns-app.na2na.dev)
...
知財で飯を食うものとしてライセンスは正しくハンドリングしましょう。
-->
...
- trivy による脆弱性スキャン
  ![trivy](https://misskey.na2na.dev/media/media/5db28bdb-cc68-49bb-8ad7-525ae3a94d67.png)
...
const useHooks = (
	...
	HogeComponent: FC<HogeComponentProps>
) => {
	React.createElement(HogeComponent, {
		onClick: handleClick,
	});
};
```
...
<!--このようにすれば、テスト時にコンポーネントの差し替えができます。-->
...
- バックエンド  
   ...
   気持ち的には ECS ですが、コスト面から今回は採用見送りです。  
  sail コンテナを、Production 用に魔改造してあります。
...
- バックエンド  
   sail コンテナを、Production 用に魔改造してあります。  
   デフォルトの`laravel.test`コンテナは並列リクエストを処理できません。  
   `php -S`で利用するビルトインサーバを用いているためです。  
   間違っても本番環境で使ってはいけません。
...
![BackendCD](https://misskey.na2na.dev/media/media/86ad3548-3194-442e-a0fb-614529647823.png)
